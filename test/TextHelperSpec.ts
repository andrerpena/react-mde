...
describe("getSurroundingWord", () => {
    ...
    it("Basic usage", () => {
        ...
        assert.strictEqual(result.position.start, 4);
        assert.strictEqual(result.position.end, 8);
    });
    ...
    it("text beginning", () => {
        ...
        assert.strictEqual(result.position.start, 0);
        assert.strictEqual(result.position.end, 3);
    });

    it("text ending", () => {
        const result = getSurroundingWord("bob went to school", 18);
        assert.strictEqual(result.word, "school");
        assert.strictEqual(result.position.start, 12);
        assert.strictEqual(result.position.end, 18);
    });
    ...
    it("text beginning with line break", () => {
        ...
        assert.strictEqual(result.word, "bob");
        assert.strictEqual(result.position.start, 1);
        assert.strictEqual(result.position.end, 4);
    });
    ...
    it("within spaces", () => {
        ...
        assert.strictEqual(result.position.start, 1);
        assert.strictEqual(result.position.end, 1);
    });

    it("within spaces 2", () => {
        const result = getSurroundingWord("   ", 2);
        assert.strictEqual(result.word, "");
        assert.strictEqual(result.position.start, 2);
        assert.strictEqual(result.position.end, 2);
    });
